---
title: "Improved MSD example for Wieczorek and Lei paper"
...
Create Table 1 for
"Model-Selection Properties of Forward Selection and
Sequential Cross-Validation for High-Dimensional Regression"
by Wieczorek and Lei.

Code has been revised based on reviewer feedback that it's VERY slow.
The original version ran sequentially with R's basic `MASS:addterm()`
and all the machinery of creating `lm` objects.
Now, we use `leaps` package to generate the path very quickly
and extract only the betahats needed for predictions,
which is much faster.

```{r}
suppressWarnings(library(leaps)) ## for regsubsets()
suppressWarnings(library(Matrix)) ## for sparse matrix multiplication

## Load in full learning and holdout sets:
## Learning set = first 463715 cases of original MSD,
## Holdout set  = last   51630 cases of original MSD
## (as recommended by the dataset creators).
...
For SeqCV, going completely sequentially (train & test one $\hat{K}$ at a time)
...
Without writing our own FORTRAN code, we can compromise by going in batches:
train a path of `Kmod=30` $\hat{K}$ values at a time,
then test each of them sequentially one at a time,
and move on to the next 30 if no local min is found, etc.
...
rep_Full1090 = replicate(nreps, {
  time_Full1090 = system.time({
    ...
    Bhat1090 = Matrix(sapply(1:p, function(x) fillbeta(x, fs1090)),
                      sparse = TRUE)
...
mean(rep_Full1090[1, ])
2*sd(rep_Full1090[1, ])/sqrt(nreps)
...
rep_Full9010 = replicate(nreps, {
  time_Full9010 = system.time({
    fs9010 = regsubsets(x = X_MSD_train_9010, y = Y_MSD_train_9010,
                        nvmax = p, method = "forward")
    Bhat9010 = Matrix(sapply(1:p, function(x) fillbeta(x, fs9010)),
                      sparse = TRUE)
...
mean(rep_Full9010[1, ])
2*sd(rep_Full9010[1, ])/sqrt(nreps)
...
```{r}
## We only calculate RMSEs, not timings too.
